...
public:
    ...
    */
    void solve(){
        ...
        std::vector< std::vector<Parent> > p(n, std::vector<Parent>(n)); // used to store the predecessor for path reconstruction
        for(int i = 0; i < n; i++){
            for(int j = 0; j <= i; j++){
                if(i == 0){
                    dp[i][j] = mat[i][j];
        ...
        Parent currParent = Parent(sx, sy);
        while(currParent.x != -1){
            seq.push_back(mat[currParent.x][currParent.y]);
            Parent nextParent = p[currParent.x][currParent.y];

 ...
 * reference because it is used in quicksort()  
*/
void mypartition(vector<int> &arr, int start, int end,
int &l_pivot_idx, int &r_pivot_idx){
  ...
  while(idx <= r_idx){
    ...
    if(arr[idx] < l_pivot){
      ...
      while(arr[r_idx] > r_pivot && idx < r_idx) r_idx--;
...
void randomized_partition(vector<int> &arr, int start, int end,
int& l_pivot_idx, int& r_pivot_idx){
  ...
  myswap(arr[p1], arr[start]);
  myswap(arr[p2], arr[start+1]);
...
void quicksort(vector<int>& arr, int start, int end){
  ...
  if(start < end){
    ...
    randomized_partition(arr, start, end, l_pivot_idx, r_pivot_idx);
